...
def is_valid_event(event):
    ...
    # Verifica se amount é numérico (bool é subclasse de int, deve ser excluído)
    amount = event["amount"]
    if isinstance(amount, bool) or not isinstance(amount, (int, float)):
        return False

    # Verifica se amount é não-negativo
    return amount >= 0
