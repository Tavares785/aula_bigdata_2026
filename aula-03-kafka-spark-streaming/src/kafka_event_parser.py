...
def parse_kafka_message(raw_value):
    """
    Faz o parse e valida a estrutura basica de uma mensagem Kafka.
    """
    try:
...
def is_valid_event(event):
    """
    Verifica se o evento possui todos os campos obrigatorios
    e se amount e um numero maior ou igual a zero.
    """
    if not isinstance(event, dict):
        return False
...
def filter_valid_events(events):
    """
    Retorna somente os eventos que passam na validacao.
    """
