...
def calculate_num_blocks(file_size_mb: float, block_size_mb: int = 128) -> int:
    ...
    """
    if file_size_mb < 0:
        raise ValueError("file_size_mb deve ser maior ou igual a zero")
    if block_size_mb <= 0:
...
def simulate_block_distribution(num_blocks: int, num_datanodes: int) -> dict:
    ...
    """
    if num_blocks < 0:
        raise ValueError("num_blocks deve ser maior ou igual a zero")
    if num_datanodes <= 0:
    ...
    for block_id in range(1, num_blocks + 1):
        ...
        distribution[f"datanode-{datanode_id}"].append(block_id)
